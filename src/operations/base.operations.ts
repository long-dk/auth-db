...
export async function findById<T>(
  ...
  return accessor.findFirst({ where: { id, deletedAt: null } });
}
...
export async function findOne<T>(
  ...
  const accessor = client[model] as any;
  return accessor.findFirst({ where: { ...filter, deletedAt: null } });
}
...
export async function findMany<T>(
  ...
  const where: Record<string, unknown> = { deletedAt: null, ...customFilters };
...
export async function softDelete<T>(
  ...
  const accessor = client[model] as any;
  return accessor.update({ where: { id }, data: { deletedAt: new Date() } });
}
...
export async function countRecords(
  ...
  const accessor = client[model] as any;
  return accessor.count({ where: { deletedAt: null, ...filters } });
}
